...
public class ActorControllerJPA {
    ...
    public ActorJPA findByNameQuery(String name) {
        EntityManager em = emf.createEntityManager();
        ActorJPA toReturn = null;
        List<ActorJPA> listReturn = null;
        listReturn = (List<ActorJPA>) em.createNamedQuery("findByNameQuery", ActorJPA.class).setParameter("nume", name).getResultList();
